...
const populationArray = countries_data  
.map((a) => [a["name"], a["population"]]) 
.sort(function (a, b) { 
return b[1] - a[1] }) 
const sliceArray = populationArray.slice(0, 10) 
const populationkeys = sliceArray.map((a) => a[0]) 
const populationvalues = sliceArray.map((a) => a[1]) 
...
for (let {languages} of  countries_data) {  
 ...
 let currentWordCount = wordMap[languages[i]];  
 let count = currentWordCount ? currentWordCount : 0;  
 wordMap[languages[i]] = count + 1;  
  }  
}    
const sortArray = Object.entries(wordMap).sort((a,b) =>  b[1] - a[1])  
const sliceLangArray = sortArray.slice(0, 10) 
const languageKey = sliceLangArray.map((a) => a[0]) 
const languageValue = sliceLangArray.map((a) => a[1]) 
...
const config2 = { 
 ...
 config 
   ...
   function chartType() { 
 const hideText = document.querySelector(".spokenText") 
 hideText.style.display = "block" 
 const name = document.querySelector(".name") 
 name.innerHTML = "Populated Countries" 
 myChart.destroy() 
 myChart = new Chart ( 
   document.getElementById('myChart'), 
   config2) 
 ...
 const displayText = document.querySelector(".spokenText") 
 displayText.style.display = "block" 
 const name = document.querySelector(".name") 
 name.innerHTML = "Used Languages" 
   myChart.destroy() 
   myChart = new Chart ( 
  document.getElementById('myChart'), 
  config) 
